...
func (u *user) Valid(v *validation.Validation) {
	if strings.Index(u.Name, "admin") != -1 {
		...
		v.SetError("Name Admin", "Can't contain 'admin' in Name")
	}
	fmt.Println(1234)
}
...
func DoBeegoValidator(ctx *gin.Context) {
	...
	if !b {
		for _, err := range valid.Errors {
			...
			ctx.String(http.StatusBadRequest, "key: %s: val: %s\n", err.Key, err.Message)
			ctx.String(http.StatusBadRequest, "key: %s: val: %s\n", key_mapping[err.Key].(string), err.Message)
			return
		}
	}
	// key: Name Admin: val: Can't contain 'admin' in Name
	// key: amind invalid: val: Can't contain 'admin' in Name
