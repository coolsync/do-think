...
trait CompanyName {
    fn get_company_name(&self) -> String {
        String::from(".inc")
    }
}
...
impl CompanyName for Manager {
    fn get_company_name(&self) -> String {
        String::from(".c.c")
    }
}

// 4、default impl：可以在定义trait的时候提供默认的行为，trait的类型可以使用默认的行为
...
fn main() {
    ...
    let emp_company = emp.get_company_name();
    let mgr_company = mgr.get_company_name();
    println!("emp company name: {}", emp_company);
    println!("mgr company name: {}", mgr_company);
